...
export function calculateSeats(district: District): SeatDistribution {
  ...
  let remainingSeats: number =
    totalSeats - Object.values(seats).reduce((a, b) => a + b, 0);
...
function fuseSeats(newSeatDistribution: {
  [district: string]: { [party: string]: number };
}) {
  let newSeatDistributionTotal: { [party: string]: number } = {};
  for (const districtKey in newSeatDistribution) {
    for (const partyKey in newSeatDistribution[districtKey]) {
      ...
      newSeatDistributionTotal[partyKey] +=
        newSeatDistribution[districtKey][partyKey];
    }
  }
  return newSeatDistributionTotal;
}
...
export function TotalSeatsForBarChart(SeatDistributionTotal: {
  [party: string]: number;
}) {
  let SeatsForBarChart: {
    [party: string]: { name: string; seats: number; colour: string };
  } = {};
