...
export function getProportionalVotes(districts: DistrictVotes) {
	let votesSum: { [district: string]: number } = Object.entries(districts).reduce(
		(acc: { [district: string]: number }, [district, data]) => {
	...
	return Object.entries(districts).reduce(
		(acc: { [district: string]: { [party: string]: number } }, [district, data]) => {
...
export function getParticipationRatio(districts: DistrictVotes) {
	...
	return Object.keys(districts).reduce((acc: DistrictCounts, district) => {
		const votes = Object.entries(districts).reduce(
			(acc: DistrictCounts, [district, data]: [string, { votes: { [party: string]: number } }]) => {
				acc[district] = Object.values(data.votes).reduce((sum, current) => sum + current, 0);
				return acc;
			},
			{}
...
export function changeParticipationRatioChange(
	...
	Object.keys(proportionalVotesDistrict).forEach((key) => {
		district[key] = Math.round(
			proportionalVotesDistrict[key] * participationRatioDistrict * allowedVoters[selectedDistrict]
		);
	});
	return district;
}
